...
function save_mecparam(bfgs_minimizer)
    df = DataFrame();

    for i in 1:n_conditions
        print("\tSaving mecparameters $(i)\n");
        ...
        loss = Inf;
        try 
            for j in 1:size(condition[:X],2)
                mec_parameters = NN_parameters(condition[:X][ : ,j], bfgs_minimizer.nn_mec, nn_stmec)[1]
                push!(df, (Condition=i, MecParameters=mec_parameters, Type=type));
            end
            #solution = predict(bfgs_minimizer, condition[:u0]);
            #X̂ = Array(solution);
            #JLSO.save("$(results_folder)/$(type)-bfgs_solution_condition_$(index).jlso", :ude_solution => X̂);
    ...
    CSV.write("$(dataframe_folder)/mec_paramets.csv", df); 
